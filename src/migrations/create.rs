    ...
    -> anyhow::Result<()>
{
    ...
    let descr = 'migration: loop {
        ...
        if let Some(proposal) = &descr.proposed {
            ...
            let already_approved = proposal.operation_id.as_ref()
            ...
            if already_approved {
                ...
                println!("(approved as part as part of an earlier operation)");
            } else {
            ...
            if let Some(operation_id) = &proposal.operation_id {
                operations.push(
                    operations.last().unwrap().insert(operation_id.clone()).0
                );
            } else {
